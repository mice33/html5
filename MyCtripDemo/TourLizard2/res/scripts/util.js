    ...
    */
    ...
    util.getEnvCode = function (cUtility) {
        ...
        if (cUtility && cUtility.isInApp()) {
            //return cUtility.isPreProduction(); //1、定义堡垒环境 0、定义测试环境 2、uat环境  其他：生产环境
            var pre = parseInt(cUtility.isPreProduction());
            return isNaN(pre) || [0, 1, 2].indexOf(pre) < 0 ? 3 : pre;
        }
    ...
    util.getQueryString = function (name) {
        ...
        var r = window.location.hash.split('?')[1] ? window.location.hash.split('?')[1].match(reg) : null;
        if (r != null) return decodeURIComponent(r[2]); return null;
    };
    util.setGuid = function () {
        var ref = function () {
            return (((1 + Math.random()) * 0x10000) | 0).toString(16).substring(1);
        };
        return (ref() + ref() + "-" + ref() + "-" + ref() + "-" + ref() + "-" + ref() + ref() + ref());
    };
    ...
    util.arrayReplac=function(arr,arg1,arg2){
        var len;
        if(toString.call(arr) == CONST.OBJECT_ARRAY && (len = arr.length)){
            for(var i = 0;i<len;i++){
                if(arr[i] === arg1){
                    arr[i] = arg2;
                    break;
