...
var CtripUtil = {
     ...
     * @param {int} targetMode @required<br>
     ...
     2.开启新的WebView,title生效; 注：只支持online地址<br/>
     3.使用系统浏览器打开; 注：只支持online地址和其它App的URL Schema，例如微信的weixin://home<br/>
     4.开启新的WebView，title生效，打开webapp目录下的相对路径；注：和2对应，2打开online地址，4打开相对路径<br/>
     ...
     * @param {boolean} isShowLoadingPage  @optional 开启新的webview的时候，是否加载app的loading v6.3，该参数废弃，不再work
     * @param {JSON} meta, 页面跳转的一些配置参数,现在支持isHideNavBar字段，boolean类型，v6.2加入, isHideLoadingViewForOnlinePage v6.3加入，打开online H5页面，默认显示加载页面，该属性可以关闭。
     * @method app_open_url
     * @since v5.2
     * @author jimzhao
     * @example 
...
var CtripPage = {
   ...
   /**
    ...
    /**
     * @description 调用native api，从磁盘reload当前页面
     * @brief 从磁盘reload当前页面
     * @method app_reload_current_page
     * @author jimzhao
     * @since v6.3
     * @example
     *
     * 
        CtripPage.app_reload_current_page();

     */
    app_reload_current_page:function() {
        if (!Internal.isSupportAPIWithVersion("6.3")) {
            return;
        }

        var paramString = Internal.makeParamString("Page", "reloadCurrentPage", null, "reload_current_page");
        if (Internal.isIOS) {
            var url = Internal.makeURLWithParam(paramString);
            Internal.loadURL(url);
        } 
        else if (Internal.isAndroid) {
            window.Page_a.reloadCurrentPage(paramString);
        } 
        else if (Internal.isWinOS) {
            Internal.callWin8App(paramString);
        } 
