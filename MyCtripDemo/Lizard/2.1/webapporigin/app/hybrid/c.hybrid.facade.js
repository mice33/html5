...
define(['cHybridShell', 'cCommonStore'], function (cHybridShell, CommonStore) {
  ...
  var METHOD_INFO = {
    ...
    APP_CHOOSE_PHOTO                   : { argnames: [
      {maxFileSize: 200},
      {maxPhotoCount: 1},
      {meta: { canEditSinglePhoto: false }}
    ] },
    APP_FINISHED_LOGIN                 : { argnames: [ 'userInfo' ] },
    APP_FINISHED_REGISTER              : { argnames: [ 'userInfo' ] },
    APP_GET_DEVICE_INFO                : {},
    APP_LOG_GOOGLE_REMARKTING          : { argnames: [
      {url: function () {
    ...
    LOCATE: {
      ...
      parseCallback: function (options) {
        return function (params, err) {
          var errcode;
          if (err) errcode = err.error_code || err.number;
          try {
            options.success(params, errcode);
          } catch (ex) {
            options.error(params,errcode);
          }
        };
      }
    },
  ...
  var _ME = {
    ...
    name2var  : function (name) {
      return 'METHOD_' + name;
    },

    // 方法标签名（Lizard）转换成方法索引名
    var2name  : function (varname) {
      return varname.substr(7);
    },
