 ...
 * @version V1.0
 */
define(['cUserModel', 'cUtilValidate', 'cUtilPath','cUtilCryptBase64', 'cCommonStore','cUtilCommon','cLocalStore','cCookieStorage'],
  function (UserModel, cUtilValidate,cUtilPath, CryptBase64, CommonStore,cUtilCommmon,cLocalStore,cCookieStorage) {
  "use strict";
  ...
  var Member = {
    ...
    autoLogin: function (opt) {
      ...
      var  shortToken = cUtilPath.getUrlParam(url, 'GetUserInfos'),
        ...
        cookieAuth = cookieStorage.get('authkey'),
        self = this;
      ...
      var sucCb = function (data) {
        ...
        if(data.UserID){
         ...
         // userStore.setAuth(data.Auth);
          ...
          (new Date(data.ExpiredTime) != "Invalid Date") && userStore.setExpireTime(data.ExpiredTime);
        }
      ...
      }else if(cookieAuth && cookieAuth != userStore.getAuth()) {
      ...
      function getClientId(){
        ...
        }else if(protocol == 'https:' && !cid ){
          cid = cUtilCommmon.createGuid();
          lstore.setItem('GUID',cid);
          headStore.setAttr('cid',cid);
        }
      }
    },
    /**
