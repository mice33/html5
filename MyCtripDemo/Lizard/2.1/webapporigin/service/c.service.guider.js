 ...
 */
define([(Lizard.app.code.is('MASTER') || Lizard.app.code.is('YOUTH') || Lizard.isHybrid) ? 'cHybridGuider' : 'cWebGuider', Lizard.app.code.is('GS')?'cHybridFacade':''], function (TGuider, Facade) {
  ...
  _.each(metheds,function(item){
    if(_.isString(item) ){
	  if (item == 'create' && Lizard.app.code.is('GS')) {
		Guider[item] = function() {
		  Facade.init();
		};
	  } else {
