 ...
 */
define(['cHybridFacade', 'cHybridShell','cUtilPath'], function (Facade, Hish,Path) {
  "use strict";
  ...
  var HybridGuider = {
      ...
      var model = {
        ...
        h5: function () {
          if (options && options.url) {
            Facade.request({ name: Facade.METHOD_OPEN_URL, openUrl: options.url, targetMode: 2, title: options.title, pageName: options.pageName, isShowLoadingPage: options.isShowLoadingPage, meta: options.meta});
          }
        },
        browser: function () {
          if (options && options.url) {
            Facade.request({ name: Facade.METHOD_OPEN_URL, openUrl: options.url, targetMode: 3, title: options.title, pageName: options.pageName, isShowLoadingPage: options.isShowLoadingPage, meta: options.meta});
          }
        },
        open: function () {
          if (options && options.url) {
            Facade.request({ name: Facade.METHOD_OPEN_URL, openUrl: options.url, targetMode: 4, title: options.title, pageName: options.pageName, isShowLoadingPage: options.isShowLoadingPage, meta: options.meta});
          }
        },
        open_relative: function () {
          if (options && options.url) {
            Facade.request({ name: Facade.METHOD_OPEN_URL, openUrl: options.url, targetMode: 5, title: options.title, pageName: options.pageName, isShowLoadingPage: options.isShowLoadingPage, meta: options.meta});
          }
        }
      };
     ...
     * @param {pageid} [opt.businessCode] 业务号码,默认取view中的businessCode属性
     * @param [pageId] [opt.pageId] ubt pageId,hybrid，默认取view.hpageid
	   */
    callService: function (options) {
     //add by byl 在此处添加pageid以及businessCode参数
     var curView,businessCode,tel,hPageId;
     if(Lizard && Lizard.instance && Lizard.instance.curView){
       curView = Lizard.instance.curView;
     }
     businessCode = (options && options.businessCode) || (curView && curView.businessCode) || '';
     tel = (options && options.tel) || '';
     hPageId = (options && options.pageId) || (curView && curView.hpageid) || '';
     if(businessCode){
       Facade.request({ name: Facade.METHOD_CALL_SERVICE_CENTER, tel: tel, pageId : hPageId, businessCode : businessCode });
     }else{
  ...
  HybridGuider.pay = {
    ...
    callPay: function(options) {
      console.log('before options: ' + options.param);
      //add by byl  直连时不需要修改
      if(Lizard.isHybrid){
        try{
          var prefix="file://webapp/",
            re = /(token=)([^&]*)/i,
            path = options.param,
            token = (path.match(re))[2],
            tokenStr = CtripTool.base64Decode(decodeURIComponent(token));

          console.log('before paramStr: ' + tokenStr);
          var param = JSON.parse(tokenStr);
          param.from =  Path.formatHybridUrl(param.from,prefix);
          param.rback = Path.formatHybridUrl(param.rback,prefix);
          param.sback = Path.formatHybridUrl(param.sback,prefix);
          param.eback = Path.formatHybridUrl(param.eback,prefix);

          var paramStr = JSON.stringify(param);
          console.log('after paramStr: ' + paramStr);
          paramStr = encodeURIComponent(CtripTool.base64Encode(paramStr));
          options.param = path.replace(token,paramStr);

          console.log('after options: ' + options.param);
